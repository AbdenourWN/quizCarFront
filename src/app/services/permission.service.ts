import { Injectable } from '@angular/core';
import { StorageService } from './storage.service';
import {
  HttpClient,
  HttpErrorResponse,
  HttpHeaders,
} from '@angular/common/http';
import {
  BehaviorSubject,
  catchError,
  Observable,
  Subject,
  throwError,
} from 'rxjs';
import { webSocket, WebSocketSubject } from 'rxjs/webSocket';

export interface Permission {
  _id?: string;
  Get?: Boolean;
  Post?: Boolean;
  Delete?: Boolean;
  Patch?: Boolean;
  role?: string;
  feature?: string;
}

@Injectable({
  providedIn: 'root',
})
export class PermissionService {
  private permissionSubject = new BehaviorSubject<Permission[]>([]);
  public socket$!: WebSocketSubject<any>;
  private permissions: Permission[] = [];
  constructor(private storage: StorageService, private http: HttpClient) {
    this.initWebSocket();
  }

  watch(): Observable<Permission[]> {
    return this.permissionSubject.asObservable();
  }

  getPermissions() {
    let options = this.getStandardOptions();
    this.http
      .get<Permission[]>('http://localhost:3000/api/permission', options)
      .pipe(catchError(this.handleError))
      .subscribe((res: any) => {
        this.permissions = res;
        this.permissionSubject.next(res);
      });
  }

  private initWebSocket() {
    const token = this.storage.getToken(); // Assuming you want to use the token later for authentication
    const url = `ws://localhost:3000/api/permission?token=${token}`;
    this.socket$ = webSocket(url);
    this.socket$.subscribe({
      next: (msg) => {
        if (msg.url === '/api/permission') {
          if (msg.method === 'create') {
            this.permissions = [...this.permissions, msg.permission];
            this.permissionSubject.next(this.permissions);
          } else if (msg.method == 'update') {
            this.permissions.forEach((el, index) => {
              if (el._id === msg.permission._id) {
                this.permissions[index] = msg.permission;
                return;
              }
            });
            this.permissionSubject.next(this.permissions);
          } else {
            this.permissions = this.permissions.filter((el) => el._id !== msg.permission._id);
            this.permissionSubject.next(this.permissions);
          }
        }
      }, // Called whenever there is a message from the server.
      error: (err) => console.log(err), // Called if at any point WebSocket API signals some kind of error.
      complete: () => console.log('complete'), // Called when connection is closed (for whatever reason).
    });
  }

  private getStandardOptions(): any {
    const token = this.storage.getToken();
    return {
      headers: new HttpHeaders({
        'Content-Type': 'application/json',
        Authorization: `Bearer ${token}`,
      }),
    };
  }

  private handleError(error: HttpErrorResponse) {
    if (error.status === 0) {
      console.error(
        'There is an issue with the client or network',
        error.error
      );
    } else {
      console.error('Server-Side-Error: ', error.error);
    }
    return throwError(
      () =>
        new Error(
          'Cannot retrieve permissions from the server. Please try again.'
        )
    );
  }

  getPermission(id: string) {
    let options = this.getStandardOptions();
    return this.http
      .get(`http://localhost:3000/api/permission/${id}`, options)
      .pipe(catchError(this.handleError));
  }
  updatePermission(id: string, payload: Permission | null) {
    let options = this.getStandardOptions();
    return this.http
      .patch(`http://localhost:3000/api/permission/${id}`, payload, options)
      .pipe(catchError(this.handleError));
  }
  deletePermission(id: string) {
    let options = this.getStandardOptions();
    return this.http
      .delete(`http://localhost:3000/api/permission/${id}`, options)
      .pipe(catchError(this.handleError));
  }
}
